...
- Timeline: every scene and event with a story `date` on one axis, in threads by label, events dashed, ranges as bars from `date_end`, and a Written track from `created`. Click a pin to open the scene, drag it to change the date at the precision the note uses. Undated scenes wait in a tray; click one to give it a date. Dates may be a year, a year and month, or a full date, any era.
